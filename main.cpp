...
static int progress_cb(void *clientp, curl_off_t dltotal, curl_off_t dlnow,
  ...
  if (abort) {
    std::cout << "[cancel] Aborting download of " << url << " after " << dlnow / 1024 << " KiB" << std::endl;
    return 1;
  }
  return 0;
}
...
static void worker_thread(int id, const std::vector<std::string> &urls, std::chrono::seconds duration) {
  ...
  while (std::chrono::steady_clock::now() < deadline) {
    ...
    if (!handles.empty() && pick10(rng) == 0) {
      ...
      CURL *e = handles[idx];
      std::cout << "[cancel] thread " << id << " removing handle" << std::endl;
      curl_multi_remove_handle(multi, e);
      curl_easy_cleanup(e);
...
int main() {
  ...
  curl_global_cleanup();
  std::cout << "Finished stress run." << std::endl;
  return 0;
}
